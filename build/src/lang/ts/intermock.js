    ...
    switch (types[normalizedTypeName].kind) {
        ...
        case typescript_1.default.SyntaxKind.ExportSpecifier:
            if (options.importsResolver) {
                options.importsResolver(sourceFile, output, types, normalizedTypeName, property, options);
            }
            break;
        default:
...
function normalizeNamespaceTypeName(typeName) {
...
function traverseInterfaceMembers(node, output, sourceFile, options, types) {
    ...
    const processPropertySignature = (node) => {
        ...
        if (typeName === "Date" && kind == typescript_1.default.SyntaxKind.TypeReference) {
            output[property] = generatePrimitive(property, typescript_1.default.SyntaxKind.StringKeyword, options, "date.past");
            return;
        }
        switch (kind) {
            case typescript_1.default.SyntaxKind.TypeReference:
