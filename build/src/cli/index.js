...
const resolve_file_imports_1 = require("../lib/resolve-file-imports");
...
function main() {
    ...
    const importsResolver = options.resolveFileImports ? resolve_file_imports_1.setImportExportSpecifier : undefined;
    return read_files_1.readFiles(options.files).then((files) => {
        try {
            const result = intermock_1.mock({ files, interfaces, isFixedMode, output, importsResolver });
            console.log(result);
        }
        catch (err) {
